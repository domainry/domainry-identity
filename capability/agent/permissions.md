...
3. Choose exactly one of `all`, `owner`, `org`, `org_child`, or `target_org`. If none expresses the requirement, report a Contract Gap instead of inventing a custom predicate.
4. Declare the Role in `domainry.model/v3`; Identity owns live users and assignments, while Runtime evaluates the exact grant on every request.
5. If one human Role is the initial Workspace administrator, reference that Role from `project.initial_workspace_administrator_role`; Runtime owns Workspace creation and first assignment.
6. Verify one allowed call, one call without the exact permission, one out-of-scope row, and unchanged durable state after a rejected write.
...
`owner` means Runtime’s immutable creator, not a mutable assignee. Reassignment requirements need a truthful organization boundary or a named Operation that validates the assignee inside wider authorized scope.
...
Field permissions, menus, users, and live Role assignment are separate Identity-owned capabilities. Do not translate old Builder compact-map examples into `domainry.model/v3` JSON.
