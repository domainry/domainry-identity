...
func TestProjectProfileExtensionPublisherOverlaysSourceMetadataAndDeepCopies(t *testing.T) {
	...
	source := []identitysdk.ProjectProfileExtension{{
		ContractVersion: "identity-profile-extension", MinReaderVersion: "identity-profile-extension-reader",
		...
		ProfileTabLabels: map[string]string{"employment": "Employment"}, ProfileTabFields: map[string][]string{"employment": {"employee_no"}},
	}}
	...
	if published.IdentityRelationField != "identity_user_id" || published.BusinessIdentity.ActiveStatusValues[0] != "active" || published.BusinessIdentity.Claims[0].FieldKey != "store_code" || published.BindingLifecycle.ClaimProofs[0].FieldKey != "employee_no" || published.ProfileTabLabels["employment"] != "Employment" || published.ProfileTabFields["employment"][0] != "employee_no" {
