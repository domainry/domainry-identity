...
func (h *MetadataHandler) localizedTextImportAllowedKeys(ctx context.Context, principal identitymodel.Principal) (map[string]bool, error) {
	...
	menus, err := h.identityCatalog.ListMenus(requestcontext.WithWorkspaceID(ctx, principal.WorkspaceID))
	if err != nil {
		return nil, err
	}
	for _, menu := range menus {
		...
		for _, property := range []string{"label", "name", "description"} {
			allowed[localizedTextImportKey("menu", key, property)] = true
		}
	}
	for _, permission := range h.identityCatalog.ListPermissions(ctx) {
		key := strings.TrimSpace(permission.Key)
		if key == "" {
			continue
		}
		for _, property := range []string{"label", "name", "description", "resource_label"} {
			allowed[localizedTextImportKey("permission", key, property)] = true
		}
	}
	return allowed, nil
}
