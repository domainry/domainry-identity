...
type SafeStatus struct {
	...
	ServiceReplicaCount int    `json:"service_replica_count"`
	TLS                 bool   `json:"tls"`
...
func (p ConnectionProfile) SafeStatus() SafeStatus {
	return SafeStatus{Backend: p.Backend, Mode: p.Mode, Schema: p.Schema, MaxOpenConns: p.MaxOpenConns, MaxIdleConns: p.MaxIdleConns, ServerMaxConns: p.ServerMaxConns, ReservedConns: p.ReservedConns, ServiceReplicaCount: p.ServiceReplicaCount, TLS: p.TLS, TLSVerified: p.TLSVerified, PreparedStatements: p.PreparedStatements, MigrationConfigured: p.MigrationConfigured, MigrationMode: p.MigrationMode, RLSEnabled: p.RLSEnabled}
}
