...
func TestPostgresProfileUsesGenericDefaults(t *testing.T) {
	profile, err := NewConnectionProfile(config.Config{
		DatabaseDSN:          "postgres://runtime:secret@localhost:5432/identity?sslmode=disable",
...
func TestPostgresTransactionPoolerModeIsProviderNeutral(t *testing.T) {
	profile, err := NewConnectionProfile(config.Config{
		DatabaseDSN:            "postgres://runtime:secret@pool.example.test:9999/identity?sslmode=require",
...
func TestPostgresProfileDoesNotInterpretProviderCredentialsOrRoles(t *testing.T) {
	for _, dsn := range []string{
		"postgres://postgres:secret@localhost:5432/identity?sslmode=disable",
		"postgres://runtime:sb_secret_opaque@localhost:5432/identity?sslmode=disable",
	} {
...
func TestPostgresProfileValidatesGenericConnectionSettings(t *testing.T) {
	tests := []struct {
		...
		{name: "wrong database", cfg: config.Config{DatabaseDSN: "postgres://u:p@localhost/business"}, want: "identity database"},
		{name: "unsafe schema", cfg: config.Config{DatabaseDSN: "postgres://u:p@localhost/identity", DatabaseSchema: "bad-name"}, want: "safe SQL identifier"},
		{name: "idle exceeds open", cfg: config.Config{DatabaseDSN: "postgres://u:p@localhost/identity", DatabaseMaxOpenConns: 2, DatabaseMaxIdleConns: 3}, want: "must not exceed"},
		{name: "unknown mode", cfg: config.Config{DatabaseDSN: "postgres://u:p@localhost/identity", DatabaseConnectionMode: "vendor_pool"}, want: "must be direct"},
	}
