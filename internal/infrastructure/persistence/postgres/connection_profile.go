...
func NewConnectionProfile(cfg config.Config) (ConnectionProfile, error) {
	...
	if !strings.EqualFold(strings.TrimSpace(connConfig.Database), "identity") {
		return ConnectionProfile{}, fmt.Errorf("PostgreSQL DATABASE_DSN must select the identity database")
	}
	migrationConfig, err := parseMigrationConfig(cfg, connConfig)
	if err != nil {
		return ConnectionProfile{}, err
	}
	if migrationConfig != nil && !strings.EqualFold(strings.TrimSpace(migrationConfig.Database), "identity") {
		return ConnectionProfile{}, fmt.Errorf("PostgreSQL DATABASE_MIGRATION_DSN must select the identity database")
	}
	...
	replicaCount := cfg.ServiceReplicaCount
	if replicaCount <= 0 {
		replicaCount = 1
	}
	if cfg.DatabaseMaxConnections > 0 {
		...
		if available <= 0 || maxOpen*replicaCount > available {
			return ConnectionProfile{}, fmt.Errorf("Identity database pool budget exceeds DATABASE_MAX_CONNECTIONS after reserved connections")
		}
	}
	...
	return ConnectionProfile{
		...
		ServiceReplicaCount: replicaCount,
