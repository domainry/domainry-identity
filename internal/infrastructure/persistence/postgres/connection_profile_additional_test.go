...
func TestPostgresProfileValidationMatrix(t *testing.T) {
	...
	profile, err := NewConnectionProfile(config.Config{DatabaseDSN: "postgres://user:secret@localhost/identity?sslmode=verify-full", DatabaseConnectTimeout: 3 * time.Second, DatabaseMaxConnections: 50, DatabaseReservedConnections: 5, DatabaseMaxOpenConns: 10, ServiceReplicaCount: 2, Environment: "production", DatabaseRLSEnabled: true})
	if err != nil || profile.ConnectTimeout != 3*time.Second || profile.MaxOpenConns != 10 || profile.ServiceReplicaCount != 2 || !profile.TLSVerified || !profile.RLSEnabled {
...
func TestPostgresProfileOpenAndWorkspaceInitializer(t *testing.T) {
	...
	profile, err := NewConnectionProfile(config.Config{DatabaseDSN: "postgres://user:secret@localhost/identity?sslmode=disable", DatabaseMaxOpenConns: 7, DatabaseMaxIdleConns: 3, DatabaseConnMaxLifetime: time.Minute, DatabaseConnMaxIdleTime: 30 * time.Second, DatabaseRLSEnabled: true})
	if err != nil {
		t.Fatal(err)
	}
