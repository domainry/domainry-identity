...
func TestPostgresProfileValidationMatrix(t *testing.T) {
	base := config.Config{DatabaseDSN: "postgres://user:secret@localhost:5432/identity?sslmode=disable"}
	tests := []struct {
		name string
		...
		{name: "production tls unverified", edit: func(cfg *config.Config) {
			...
			cfg.DatabaseDSN = "postgres://user:secret@localhost/identity?sslmode=require"
		}, want: "verification"},
	}
	...
	profile, err := NewConnectionProfile(config.Config{DatabaseDSN: "postgres://user:secret@localhost/identity?sslmode=verify-full", DatabaseConnectTimeout: 3 * time.Second, DatabaseMaxConnections: 50, DatabaseReservedConnections: 5, DatabaseMaxOpenConns: 10, ServiceReplicaCount: 2, Environment: "production"})
	if err != nil || profile.ConnectTimeout != 3*time.Second || profile.MaxOpenConns != 10 || profile.ServiceReplicaCount != 2 || !profile.TLSVerified {
		t.Fatalf("profile=%#v err=%v", profile, err)
	}
	defaults, err := NewConnectionProfile(base)
	if err != nil || defaults.MaxOpenConns != 10 || defaults.ServiceReplicaCount != 1 {
...
func TestPostgresMigrationProfileValidationAndOpen(t *testing.T) {
	base := config.Config{DatabaseDSN: "postgres://query:secret@localhost/identity?sslmode=disable"}
	...
	production := config.Config{Environment: "production", DatabaseDSN: "postgres://query:secret@localhost/identity?sslmode=verify-full", DatabaseMigrationDSN: "postgres://migrator:secret@localhost/identity?sslmode=disable"}
	...
	production.DatabaseMigrationDSN = "postgres://migrator:secret@localhost/identity?sslmode=require"
	...
	cfg := base
	cfg.DatabaseMigrationDSN = "postgres://migrator:secret@localhost/identity?sslmode=disable"
	...
	_ = db.Close()
	parsed, err := parseMigrationConfig(config.Config{DatabaseMigrationDSN: "postgres://migrator:secret@localhost/identity?sslmode=disable"}, nil)
	if err != nil || parsed == nil || parsed.Database != "identity" {
		t.Fatalf("migration config without query=%#v err=%v", parsed, err)
	}
	parsed, err = parseMigrationConfig(config.Config{Environment: "production", DatabaseMigrationDSN: "postgres://migrator:secret@localhost/identity?sslmode=verify-full"}, nil)
...
func TestPostgresProfileOpenAndWorkspaceInitializer(t *testing.T) {
	...
	profile, err := NewConnectionProfile(config.Config{DatabaseDSN: "postgres://user:secret@localhost/identity?sslmode=disable", DatabaseMaxOpenConns: 7, DatabaseMaxIdleConns: 3, DatabaseConnMaxLifetime: time.Minute, DatabaseConnMaxIdleTime: 30 * time.Second})
	if err != nil {
		t.Fatal(err)
	}
...
func TestPostgresRootCertificateAndTLSHelpers(t *testing.T) {
	plain, err := pgx.ParseConfig("postgres://user:secret@localhost/identity?sslmode=disable")
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := NewConnectionProfile(config.Config{DatabaseDSN: "postgres://user:secret@localhost/identity?sslmode=disable", DatabaseSSLRootCert: "missing"}); err == nil || !strings.Contains(err.Error(), "requires TLS") {
		t.Fatalf("query root cert propagation=%v", err)
	}
	tlsConfig, err := pgx.ParseConfig("postgres://user:secret@localhost/identity?sslmode=verify-full")
	if err != nil {
		t.Fatal(err)
	}
	...
	cfg := config.Config{DatabaseDSN: "postgres://user:secret@localhost/identity?sslmode=verify-full", DatabaseMigrationDSN: "postgres://migrator:secret@localhost/identity?sslmode=verify-full", DatabaseSSLRootCert: validPath}
