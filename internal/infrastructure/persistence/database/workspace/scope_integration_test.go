...
func TestFreshRuntimeDoesNotInitializeHistoricalMigrationReportTables(t *testing.T) {
	store, err := database.OpenContext(t.Context(), config.Config{DatabaseDriver: "sqlite", DBPath: filepath.Join(t.TempDir(), "fresh-runtime.db"), IdentityDataSecretKey: "fresh-runtime-key"})
	if err != nil {
		t.Fatal(err)
	}
	defer store.Close()
...
func TestWorkspaceScopeMigrationReportsLegacyRowsWithoutBackfill(t *testing.T) {
	store, err := database.OpenContext(t.Context(), config.Config{DatabaseDriver: "sqlite", DBPath: filepath.Join(t.TempDir(), "workspace-migration.db"), IdentityDataSecretKey: "workspace-migration-key"})
	if err != nil {
		t.Fatal(err)
	}
	defer store.Close()
	...
	if err := store.DB().QueryRowContext(t.Context(), `SELECT COUNT(*) FROM _schema_migrations WHERE path = ?`, "identity_schema_"+database.CurrentIdentitySchemaVersion).Scan(&dirtyMigrationCount); err != nil || dirtyMigrationCount != 0 {
