...
func TestProductionPersistenceDMLUsesDomainryORM(t *testing.T) {
	persistenceRoot := filepath.Clean(filepath.Join("..", ".."))
	rawDML := regexp.MustCompile(`\b(?:INSERT(?:\s+OR\s+[A-Z]+)?\s+INTO|UPDATE(?:\s+OR\s+[A-Z]+)?\s|DELETE\s+FROM)\b`)
	violations := []string{}
	err := filepath.WalkDir(persistenceRoot, func(path string, entry os.DirEntry, walkErr error) error {
		if walkErr != nil {
			return walkErr
		}
		...
		ast.Inspect(parsed, func(node ast.Node) bool {
			...
			if unquoteErr == nil && rawDML.MatchString(value) {
	...
	if len(violations) > 0 {
		t.Fatalf("production persistence DML must be rendered by domainry-orm: %s", strings.Join(violations, ", "))
	}
}
