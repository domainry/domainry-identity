...
func TestEnsureWorkspaceRLSGuardsAndSuccess(t *testing.T) {
	...
	status := store.WorkspaceRLSStatus(t.Context())
	if !status.Enabled || !status.Forced || status.ApplicationRole != "runtime_user" || status.RoleOwnsTable || status.RoleBypassRLS || status.PolicyVersion != CurrentIdentityWorkspaceRLSPolicyVersion || len(status.CoveredTables) != 2 || len(status.MissingTables) != 0 || len(script.execs) != 10 {
...
func TestWorkspaceTablesScanAndRowsErrors(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if _, err := (postgrespersistence.Dialect{}).InspectWorkspaceRLS(t.Context(), db, renderer, "public", "runtime_user", CurrentIdentityWorkspaceRLSPolicyVersion); err == nil {
...
func TestSetLocalWorkspaceRLSContext(t *testing.T) {
	renderer := postgrespersistence.Dialect{}.SQLDialect().WithSchema("public")
	if err := postgrespersistence.SetLocalWorkspaceRLSContext(t.Context(), nil, renderer, "workspace-1", "actor-1"); err == nil {
	...
	if err := postgrespersistence.SetLocalWorkspaceRLSContext(ctx, tx, renderer, requestcontext.WorkspaceID(ctx), requestcontext.ActorID(ctx)); err != nil {
	...
	defer tx.Rollback()
	if err := postgrespersistence.SetLocalWorkspaceRLSContext(ctx, tx, renderer, requestcontext.WorkspaceID(ctx), requestcontext.ActorID(ctx)); err == nil || !strings.Contains(err.Error(), "transaction-local") {
		t.Fatalf("set context error=%v", err)
	}
}

func newWorkspaceRLSTestStore(dialect dialect, db, migrationDB *sql.DB, mode string) *IdentityStore {
	return &IdentityStore{db: db, migrationDB: migrationDB, dialect: dialect, databaseSchema: "public", config: config.Config{DatabaseRLSEnabled: true, DatabaseMigrationMode: mode}, postgresProfile: &postgrespersistence.ConnectionProfile{}, postgresCapabilities: postgrespersistence.Capabilities{User: "runtime_user"}}
}
...
func (c *workspaceRLSConn) QueryContext(_ context.Context, query string, args []driver.NamedValue) (driver.Rows, error) {
	...
	if strings.Contains(query, "_identity_rls_policies") {
