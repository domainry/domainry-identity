...
func TestApplyMigrationFileSQLFailures(t *testing.T) {
	...
	tests := []struct {
		...
		{"postgres search path", databaseSQLState{execSteps: []databaseSQLExecStep{{rows: 1}, {err: errDatabaseSQL}}}, func(store *IdentityStore) { store.dialect = postgres.NewEngine() }, "search path"},
		{"postgres lock timeout", databaseSQLState{execSteps: []databaseSQLExecStep{{rows: 1}, {rows: 1}, {err: errDatabaseSQL}}}, func(store *IdentityStore) {
			store.dialect = postgres.NewEngine()
			store.config.DatabaseLockTimeout = time.Second
		}, "lock timeout"},
		{"postgres statement timeout", databaseSQLState{execSteps: []databaseSQLExecStep{{rows: 1}, {rows: 1}, {err: errDatabaseSQL}}}, func(store *IdentityStore) {
			store.dialect = postgres.NewEngine()
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			store := identitySchemaStore(t, &test.state)
			if test.prepare != nil {
				test.prepare(store)
			}
