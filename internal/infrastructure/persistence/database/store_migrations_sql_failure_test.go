...
func TestApplyMigrationFileSQLFailures(t *testing.T) {
	...
	tests := []struct {
		...
		{"postgres search path", databaseSQLState{execSteps: []databaseSQLExecStep{{rows: 1}, {err: errDatabaseSQL}}}, func(store *IdentityStore) { store.engine = postgres.NewEngine() }, "search path"},
