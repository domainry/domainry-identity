...
func (r MetadataStore) publishDefinition(ctx context.Context, scope identitymodel.SystemScope, resourceType, resourceKey string, req metadatamodel.MetadataDefinitionUpsertRequest, audit *auditmodel.AuditEvent, publication *metadatamodel.MetadataDefinitionPublication) (metadatamodel.MetadataDefinition, error) {
	...
	if moduleOwned {
		if err := r.replaceMetadataModuleDefinitionTx(ctx, tx, definition, req.ExpectedSchemaHash); err != nil {
			_ = tx.Rollback()
...
func (r MetadataStore) applyDefinitionUpsert(ctx context.Context, tx *sql.Tx, mutation metadatamodel.MetadataDefinitionMutation) (metadatamodel.MetadataDefinition, error) {
	...
	if moduleOwned {
		if err := r.replaceMetadataModuleDefinitionTx(ctx, tx, definition, mutation.Request.ExpectedSchemaHash); err != nil {
			return metadatamodel.MetadataDefinition{}, err
		}
	} else {
...
func (r MetadataStore) applyDefinitionArchive(ctx context.Context, tx *sql.Tx, mutation metadatamodel.MetadataDefinitionMutation) (metadatamodel.MetadataDefinition, error) {
	...
	if metadataModuleOwnsDefinition(mutation.ResourceType) {
		repository, err := r.metadataModuleDefinitionStore()
		if err != nil {
			return metadatamodel.MetadataDefinition{}, err
		}
		disabled, err := repository.DisableDefinitionWithExecutor(ctx, tx, mutation.ResourceType, mutation.ResourceKey, now, &expected)
		if err != nil {
			return metadatamodel.MetadataDefinition{}, fmt.Errorf("archive %s %s: %w", mutation.ResourceType, mutation.ResourceKey, err)
		}
		if !disabled {
			current, found, lookupErr := repository.GetDefinitionWithExecutor(ctx, tx, mutation.ResourceType, mutation.ResourceKey)
			if lookupErr != nil {
				return metadatamodel.MetadataDefinition{}, lookupErr
			}
			currentHash := ""
			if found {
				currentHash = current.SchemaHash
			}
			return metadatamodel.MetadataDefinition{}, &metadatamodel.MetadataDefinitionConflictError{ResourceType: mutation.ResourceType, ResourceKey: mutation.ResourceKey, ExpectedHash: expected, CurrentHash: currentHash}
		}
...
func (r MetadataStore) RollbackDefinition(ctx context.Context, scope identitymodel.SystemScope, resourceType, resourceKey string, request metadatamodel.MetadataDefinitionRollbackRequest, audit auditmodel.AuditEvent, publication *metadatamodel.MetadataDefinitionPublication) (metadatamodel.MetadataDefinition, error) {
	...
	if moduleOwned {
		...
		if err := r.replaceMetadataModuleDefinitionTx(ctx, tx, definition, &request.ExpectedSchemaHash); err != nil {
			return metadatamodel.MetadataDefinition{}, err
		}
	} else {
