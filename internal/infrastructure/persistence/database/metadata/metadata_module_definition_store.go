...
func (s MetadataStore) countOwnedDefinitionVersions(ctx context.Context, executor metadatapersistence.ExecutionExecutor, resourceType, resourceKey string) (int, error) {
...
func (s MetadataStore) insertOwnedDefinitionVersion(ctx context.Context, executor metadatapersistence.ExecutionExecutor, value metadatapersistence.DefinitionVersion) error {
...
func (s MetadataStore) listOwnedDefinitionVersions(ctx context.Context, executor metadatapersistence.ExecutionExecutor, resourceType, resourceKey string) ([]metadatapersistence.DefinitionVersion, error) {
	if strings.TrimSpace(resourceType) == "identity_profile_binding" {
...
func (s MetadataStore) getOwnedDefinitionVersion(ctx context.Context, executor metadatapersistence.ExecutionExecutor, resourceType, resourceKey, version string) (metadatapersistence.DefinitionVersion, bool, error) {
	if strings.TrimSpace(resourceType) == "identity_profile_binding" {
		...
		value := metadatapersistence.DefinitionVersion{ResourceType: resourceType, ResourceKey: resourceKey, SchemaVersion: version}
		var payload string
...
func (s MetadataStore) metadataModuleDefinitionStore() (metadatapersistence.ExecutorDefinitionRepository, error) {
	repository, ok := s.store.MetadataDefinitions().(metadatapersistence.ExecutorDefinitionRepository)
	if !ok {
...
func metadataDefinitionFromModule(value metadatapersistence.StoredDefinition) metadatamodel.MetadataDefinition {
...
func (s MetadataStore) replaceMetadataModuleDefinitionTx(ctx context.Context, tx *sql.Tx, value metadatamodel.MetadataDefinition, expectedHash *string) error {
	...
	result, err := repository.ReplaceDefinitionWithExecutor(ctx, tx, metadatapersistence.StoredDefinition{
		Definition:    metadatapersistence.Definition{ResourceType: value.ResourceType, Key: value.ResourceKey, ObjectKey: value.ObjectKey, Name: value.Name, Payload: append([]byte(nil), value.Payload...), SchemaHash: value.SchemaHash},
