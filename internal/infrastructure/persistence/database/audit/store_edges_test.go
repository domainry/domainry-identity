...
func TestAuditInsertEncodingAndQueryVariants(t *testing.T) {
	...
	if _, err := repository.ListAuditEventsForSystem(t.Context(), identitymodel.SystemScope{}, auditmodel.AuditEventQuery{}); err == nil {
		t.Fatal("invalid system scope accepted")
	}
	systemEvents, err := repository.ListAuditEventsForSystem(t.Context(), identitymodel.NewSystemScope(identitymodel.SystemScopeRuntimeGlobal, "test"), auditmodel.AuditEventQuery{Limit: 1})
...
func openAuditEdgeStore(t *testing.T) *database.IdentityStore {
	t.Helper()
