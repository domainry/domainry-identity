...
func TestEnsureIdentitySchemaBackfillsAuditWorkspaceOnLegacyTable(t *testing.T) {
	...
	if migratedWorkspace != identitymodel.InstallationWorkspaceID {
		t.Fatalf("migrated workspace=%q", migratedWorkspace)
	}
	events, err := NewAuditStore(store).ListAuditEvents(t.Context(), identitymodel.InstallationWorkspaceID, auditmodel.AuditEventQuery{Limit: 10})
	if err != nil || len(events) != 1 || events[0].ID != "legacy-audit" || events[0].WorkspaceID != identitymodel.InstallationWorkspaceID {
