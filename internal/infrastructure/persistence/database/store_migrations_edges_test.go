...
func openMigrationEdgeStore(t *testing.T) *IdentityStore {
	t.Helper()
...
func TestMigrationApplicationLedgerAndContextEdges(t *testing.T) {
	...
	closedStore := &IdentityStore{db: closedDB, dialect: sqlite.Dialect{}}
	_ = closedDB.Close()
...
func TestMigrationPathIdentityAndSQLHelperEdges(t *testing.T) {
	...
	rootFailure := &IdentityStore{dialect: sqlite.Dialect{}, migrationReadDir: func(path string) ([]os.DirEntry, error) {
...
func TestSQLiteMigrationLockShortCircuitSuccessAndTimeout(t *testing.T) {
	store := &IdentityStore{dialect: sqlite.Dialect{}, config: config.Config{DatabaseLockTimeout: 20 * time.Millisecond}}
