...
func TestFileMigrationsUseDedicatedManagementConnection(t *testing.T) {
	...
	store := &IdentityStore{db: queryDB, migrationDB: migrationDB, dialect: sqlite.NewEngine()}
...
func TestIdentitySchemaUsesDedicatedManagementConnection(t *testing.T) {
	...
	store := &IdentityStore{db: queryDB, migrationDB: migrationDB, dialect: sqlite.NewEngine(), config: config.Config{MigrationBackupDir: t.TempDir()}}
