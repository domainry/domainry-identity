...
func TestFileMigrationsUseDedicatedManagementConnection(t *testing.T) {
	...
	if err := store.Coordinator.Apply(t.Context(), config.Config{MigrationSQL: migrationPath}); err != nil {
		t.Fatal(err)
	}
...
func TestIdentitySchemaUsesDedicatedManagementConnection(t *testing.T) {
	...
	if sqliteTableExists(t, migrationDB, "_identity_manifest_catalog") != 1 {
		t.Fatal("Identity schema was not created on management connection")
	}
	if sqliteTableExists(t, queryDB, "_identity_manifest_catalog") != 0 {
