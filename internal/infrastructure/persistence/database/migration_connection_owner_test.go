...
func TestFileMigrationsUseDedicatedManagementConnection(t *testing.T) {
	...
	store := &IdentityStore{db: queryDB, migrationDB: migrationDB, dialect: sqlite.Dialect{}}
...
func TestIdentitySchemaUsesDedicatedManagementConnection(t *testing.T) {
	...
	store := &IdentityStore{db: queryDB, migrationDB: migrationDB, dialect: sqlite.Dialect{}, config: config.Config{MigrationBackupDir: t.TempDir()}}
