...
// a plain PostgreSQL DSN is enough to migrate, assemble Identity schema,
...
func TestPostgresRuntimePersistenceEndToEnd(t *testing.T) {
	...
	if err := store.EnsureSchema(t.Context()); err != nil {
		_ = store.Close()
		t.Fatalf("assemble Identity schema: %v", err)
	}
	...
	if err := restarted.EnsureSchema(t.Context()); err != nil {
		t.Fatalf("verify Identity schema after restart: %v", err)
	}
