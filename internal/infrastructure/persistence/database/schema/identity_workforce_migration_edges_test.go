...
func TestIdentityMigrationTableInspectionEdges(t *testing.T) {
	for _, dialect := range []string{"sqlite", "mysql", "postgres"} {
		t.Run(dialect, func(t *testing.T) {
			...
			columns, err := identityMigrationTableColumns(t.Context(), store, "identity_users")
			if err != nil || !columns["employee_no"] {
				t.Fatalf("columns=%v err=%v", columns, err)
			}
			indexes, err := identityMigrationTableIndexes(t.Context(), store, "identity_users")
	...
	for _, inspect := range []func(scriptedSchemaStore) error{
		func(store scriptedSchemaStore) error {
			_, err := identityMigrationTableColumns(t.Context(), store, "identity_users")
			return err
		},
		func(store scriptedSchemaStore) error {
			_, err := identityMigrationTableIndexes(t.Context(), store, "identity_users")
			return err
		},
	} {
	...
	if _, err := identityMigrationTableColumns(t.Context(), store, "identity_users"); err == nil {
