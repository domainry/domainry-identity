...
func TestIdempotencyColumnsRequireUniqueDatabaseConstraint(t *testing.T) {
	...
	for _, table := range tables {
		...
		for _, column := range columns {
			...
			if !unique {
				t.Errorf("idempotency schema violation: entry=%s owner=%s file=internal/infrastructure/persistence/database/schema missing_contract=UNIQUE(%s)", identity, idempotencyTableOwner(table), column)
			}
		}
	}
}

func sqliteTableColumns(store *database.IdentityStore, table string) ([]string, error) {
...
func sqliteColumnHasUniqueIndex(store *database.IdentityStore, table, targetColumn string) (bool, error) {
