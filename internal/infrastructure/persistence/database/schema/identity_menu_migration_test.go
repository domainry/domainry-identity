...
func TestIdentitySchemaDropsLegacyMenuAudienceWithoutLosingMenus(t *testing.T) {
	...
	if _, err := store.DB().ExecContext(t.Context(), `ALTER TABLE identity_menus ADD COLUMN audience TEXT NOT NULL DEFAULT 'admin_console'`); err != nil {
		t.Fatal(err)
	}
	if _, err := store.DB().ExecContext(t.Context(), `INSERT INTO identity_menus (id, workspace_id, menu_key, label, icon, route, parent_id, sort_order, status, created_at, updated_at, audience) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
	...
	if err := store.DB().QueryRowContext(t.Context(), `SELECT COUNT(*) FROM pragma_table_info('identity_menus') WHERE name = 'audience'`).Scan(&audienceColumnCount); err != nil {
		t.Fatal(err)
	}
	var menuCount int
	if err := store.DB().QueryRowContext(t.Context(), `SELECT COUNT(*) FROM identity_menus WHERE id = 'menu-1'`).Scan(&menuCount); err != nil {
		t.Fatal(err)
	}
