...
func TestWorkspaceRLSIsPostgresOnlyAndFailsClosedWithoutWorkspace(t *testing.T) {
	store, err := database.OpenContext(t.Context(), config.Config{DatabaseDriver: "sqlite", DBPath: t.TempDir() + "/rls.db", IdentityDataSecretKey: "rls-contract-key", DatabaseRLSEnabled: true})
	if err != nil {
		t.Fatal(err)
	}
	defer store.Close()
