...
func TestWorkspaceScopeMigrationReportsLegacyRowsWithoutBackfill(t *testing.T) {
	...
	if err := store.DB().QueryRowContext(t.Context(), `SELECT COUNT(*) FROM _identity_schema_migrations WHERE version = ?`, database.CurrentIdentitySchemaVersion).Scan(&dirtyMigrationCount); err != nil || dirtyMigrationCount != 0 {
