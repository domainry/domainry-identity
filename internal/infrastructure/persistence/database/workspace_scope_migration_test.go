...
func TestWorkspaceScopeMigrationReportsLegacyRowsWithoutBackfill(t *testing.T) {
	...
	if err := store.DB().QueryRowContext(t.Context(), `SELECT COUNT(*) FROM _schema_materializations WHERE version = ?`, database.CurrentIdentitySchemaVersion).Scan(&dirtyMigrationCount); err != nil || dirtyMigrationCount != 0 {
