...
func TestAuthMutationDecisionLifecycle(t *testing.T) {
	...
	request := authmodel.AuthMutationClaimRequest{
		Receipt:            authmodel.AuthMutationReceipt{WorkspaceID: "default", UseCase: " reset ", TargetID: " user ", IdempotencyKey: " key "},
	...
	claim, err := repository.TryBeginAuthMutation(t.Context(), "default", request)
	if err != nil || claim.Decision != idempotency.DecisionAcquired || claim.Receipt.UseCase != "reset" {
		t.Fatalf("claim=%#v err=%v", claim, err)
	}
	inProgress, err := repository.TryBeginAuthMutation(t.Context(), "default", request)
	...
	conflict, err := repository.TryBeginAuthMutation(t.Context(), "default", conflictRequest)
	...
	reclaimed, err := repository.TryBeginAuthMutation(t.Context(), "default", reclaimedRequest)
	if err != nil || reclaimed.Decision != idempotency.DecisionAcquired || reclaimed.Receipt.FencingToken != 2 {
		t.Fatalf("reclaimed=%#v err=%v", reclaimed, err)
	}
	completed, err := repository.CompleteAuthMutation(t.Context(), "default", authmodel.AuthMutationCompletion{
	...
	replayed, err := repository.TryBeginAuthMutation(t.Context(), "default", reclaimedRequest)
	if err != nil || replayed.Decision != idempotency.DecisionReplay {
		t.Fatalf("replayed=%#v err=%v", replayed, err)
	}
	if _, err := repository.CompleteAuthMutation(t.Context(), "default", authmodel.AuthMutationCompletion{Result: make(chan int)}); err == nil {
	...
	if defaultClaim, err := repository.TryBeginAuthMutation(t.Context(), "default", defaultRequest); err != nil || defaultClaim.Decision != idempotency.DecisionAcquired {
...
func TestAuthStoreDefaultAndValidationEdges(t *testing.T) {
	...
	cancel()
	if err := repository.UpsertIdentityCredential(cancelled, "default", identitymodel.IdentityCredential{}); !errors.Is(err, context.Canceled) {
		t.Fatalf("credential cancellation=%v", err)
	}
	if err := repository.UpsertIdentityExternalAccount(cancelled, "default", identitymodel.IdentityExternalAccount{}); !errors.Is(err, context.Canceled) {
		t.Fatalf("external cancellation=%v", err)
	}
	if err := repository.UpsertIdentityMFAFactor(cancelled, "default", identitymodel.IdentityMFAFactor{}); !errors.Is(err, context.Canceled) {
		t.Fatalf("MFA cancellation=%v", err)
	}
	if err := repository.UpsertIdentityCredential(t.Context(), "default", identitymodel.IdentityCredential{}); err == nil {
		t.Fatal("empty credential accepted")
	}
	if err := repository.UpsertIdentityCredential(t.Context(), "default", identitymodel.IdentityCredential{UserID: "user"}); err == nil {
		t.Fatal("credential without password accepted")
	}
	if _, err := repository.ListIdentityMFAFactors(t.Context(), "default", " "); err == nil {
		t.Fatal("MFA list without user accepted")
	}
	if err := repository.UpsertIdentityMFAFactor(t.Context(), "default", identitymodel.IdentityMFAFactor{}); err == nil {
		t.Fatal("empty MFA factor accepted")
	}
	if err := repository.RevokeIdentityMFAFactor(t.Context(), "default", "", "factor"); err == nil {
		t.Fatal("MFA revoke without user accepted")
	}
	if err := repository.RevokeIdentityMFAFactor(t.Context(), "default", "user", ""); err == nil {
	...
	if err := repository.RecordIdentityLoginSuccess(t.Context(), "default", credential.UserID, ""); err != nil {
		t.Fatal(err)
	}
	if err := repository.RecordIdentityLoginSuccess(t.Context(), "default", credential.UserID, "fixed"); err != nil {
		t.Fatal(err)
	}
	if err := repository.CreateAuthRefreshToken(t.Context(), "default", identitymodel.AuthRefreshToken{}); err == nil {
	...
	for _, invalid := range []identitymodel.AuthRefreshToken{
		...
		if err := repository.CreateAuthRefreshToken(t.Context(), "default", invalid); err == nil {
	...
	if _, found, err := repository.GetAuthRefreshTokenByHash(t.Context(), "default", "missing"); err != nil || found {
		t.Fatalf("missing found=%v err=%v", found, err)
	}
	if err := repository.RevokeAuthRefreshToken(t.Context(), "default", token.ID, "", "replacement"); err != nil {
		t.Fatal(err)
	}
	if count, err := repository.RevokeAuthRefreshTokensForUser(t.Context(), "default", "user", ""); err != nil || count != 1 {
		t.Fatalf("count=%d err=%v", count, err)
	}
	if err := repository.UpsertIdentityExternalAccount(t.Context(), "default", identitymodel.IdentityExternalAccount{}); err == nil {
	...
	for _, invalid := range []identitymodel.IdentityExternalAccount{
		...
		if err := repository.UpsertIdentityExternalAccount(t.Context(), "default", invalid); err == nil {
	...
	accounts, err := repository.ListIdentityExternalAccounts(t.Context(), "default", "")
