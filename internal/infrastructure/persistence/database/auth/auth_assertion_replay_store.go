...
func (s AuthStore) ClaimAuthAssertion(ctx context.Context, workspaceID, provider, issuer, assertionID string, expiresAt time.Time) (bool, error) {
	...
	if err != nil {
		var existing int
		lookupStatement, lookupArgs, lookupBuildErr := query.NewWorkspaceSelectBuilder(s.store.SQLRenderer(), "_identity_auth_assertion_replays", workspaceID).
			Projections(query.Project(query.CountAll())).Where(query.Equal("replay_hash", replayHash)).Build()
