...
func (s *Store) List(ctx context.Context, workspaceID string) ([]identitymodel.IdentityUser, error) {
	...
	statement, arguments, err := selectUsers(s.backend.SQLRenderer(), workspaceID).OrderBy(ormbuilder.Ascending("id")).Build()
	if err != nil {
...
func (s *Store) Get(ctx context.Context, workspaceID, userID string) (identitymodel.IdentityUser, bool, error) {
	...
	statement, arguments, err := selectUsers(s.backend.SQLRenderer(), workspaceID).Where(ormbuilder.Equal("id", strings.TrimSpace(userID))).Build()
	if err != nil {
