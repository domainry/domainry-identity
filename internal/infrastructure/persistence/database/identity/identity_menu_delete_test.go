...
func TestSQLIdentityMenuDeletionSurvivesReloadAndSeedSync(t *testing.T) {
	...
	if err := identityStore.RemoveIdentityMenu(t.Context(), identitymodel.InstallationWorkspaceID, menu.ID); err != nil {
...
func TestSQLIdentityAtomicMenuTreeRollback(t *testing.T) {
	...
	if err := identityStore.UpsertIdentityMenu(t.Context(), identitymodel.InstallationWorkspaceID, parent); err != nil {
		t.Fatal(err)
	}
	if err := identityStore.UpsertIdentityMenu(t.Context(), identitymodel.InstallationWorkspaceID, child); err != nil {
		t.Fatal(err)
	}
