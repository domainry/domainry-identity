...
func TestIdentityUserRemovalRollsBackEveryOwnedTable(t *testing.T) {
	...
	if err := repository.UpsertIdentityUser(t.Context(), "workspace-primary", user); err != nil {
		t.Fatal(err)
	}
	...
	if err := repository.RemoveIdentityUser(t.Context(), "workspace-primary", user.ID); err == nil {
		t.Fatal("trigger failure did not abort user removal")
	}
	if _, found, err := repository.GetIdentityUser(t.Context(), "workspace-primary", user.ID); err != nil || !found {
		t.Fatalf("user was partially removed: found=%v err=%v", found, err)
	}
	assignments, err := repository.ListIdentityUserRoleAssignments(t.Context(), "workspace-primary", user.ID)
	...
	var mfaCount int
	if err := identityStore.DB().QueryRowContext(t.Context(), "SELECT COUNT(*) FROM _identity_mfa_factors WHERE workspace_id = ? AND user_id = ?", "workspace-primary", user.ID).Scan(&mfaCount); err != nil || mfaCount != 1 {
	...
	if err := identityStore.DB().QueryRowContext(t.Context(), "SELECT COUNT(*) FROM _identity_mfa_factors WHERE workspace_id = ? AND user_id = ?", "workspace-primary", user.ID).Scan(&mfaCount); err != nil || mfaCount != 0 {
