...
func (s *IdentityStore) acquireMigrationLock(ctx context.Context, cfg config.Config) (func(), error) {
	...
	key := "domainry_identity_migrations:" + s.DatabaseSchema()
	if s.dialect.Name() == "mysql" {
		key = "domainry_identity_migrations"
	}
	started := time.Now()
...
func (s *IdentityStore) acquireSQLiteMigrationLock(ctx context.Context, cfg config.Config) (func(), error) {
