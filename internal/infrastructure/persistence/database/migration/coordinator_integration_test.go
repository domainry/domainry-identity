...
func TestMigrationLedgerRecordsStableReleaseIdentity(t *testing.T) {
	...
	if version != "004" || name != "expand_customer" || kind != "schema" || len(checksum) != 64 || serviceVersion != "2.4.0" || duration < 0 || operator != "release-bot" || instance != "identity-2" || backupID == "" {
		t.Fatalf("ledger identity version=%q name=%q kind=%q checksum=%d service=%q duration=%d operator=%q instance=%q", version, name, kind, len(checksum), serviceVersion, duration, operator, instance)
	}
}
...
func TestIdentitySchemaCompatibilityRangeBlocksNewerRelease(t *testing.T) {
	dir := t.TempDir()
	migrationPath := filepath.Join(dir, "004_current.sql")
...
func TestVerifyOnlyModeChecksFileAndRuntimeSchemaWithoutDDL(t *testing.T) {
	...
	if err := verified.EnsureSchema(t.Context()); err != nil {
		t.Fatalf("verify-only Identity schema rejected current database: %v", err)
	}
}
