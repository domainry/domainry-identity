...
func TestCompatibilityRejectsUnknownOldAndNewerSchemas(t *testing.T) {
	contract := Compatibility{ServiceVersion: "2.4.0", MinSchemaVersion: "002", MaxSchemaVersion: "004"}
	tests := []struct {
		current string
		known   bool
		state   string
...
func TestBackupEvidenceRequiresConsistentRecoveryBoundary(t *testing.T) {
	...
	evidence := BackupEvidence{
		...
		Artifacts: []Artifact{{Kind: ArtifactDatabase, Checksum: "db", Size: 1}, {Kind: ArtifactIdentityManifest, Checksum: "manifest", Size: 1}, {Kind: ArtifactSigningKeyVersions, Checksum: "signing-keys", Size: 1}, {Kind: ArtifactDataKeyVersions, Checksum: "data-keys", Size: 1}, {Kind: ArtifactService, Checksum: "service", Size: 1}},
	}
...
func TestDrillEvidenceRequiresIdentityRecoveryChecks(t *testing.T) {
	now := time.Now().UTC()
	checks := map[string]bool{"checksum": true, "schema": true, "critical_tables": true, "workspace_counts": true, "subject_counts": true, "role_assignments": true, "catalog_hashes": true, "session_disposition": true, "no_secret_export": true}
	evidence := DrillEvidence{DrillID: "drill-1", Scenario: "bad_migration", Engine: "sqlite", BackupID: "backup-1", StartedAt: now.Add(-time.Minute), CompletedAt: now, ActualRPO: 0, ActualRTO: 60, Checks: checks, Reconciliation: IdentityRestoreReconciliationPlan()}
	if err := evidence.Validate(); err != nil {
		t.Fatal(err)
	}
	delete(checks, "no_secret_export")
	if err := evidence.Validate(); err == nil {
		t.Fatal("drill without secret-export check accepted")
	}
}
