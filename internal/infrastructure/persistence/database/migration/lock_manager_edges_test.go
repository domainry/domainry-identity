...
func TestAcquireMigrationLockCoversExternalDialectsAndRelease(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			manager := NewLockManager(db, test.engine, test.engine.SQLDialect().WithSchema("runtime"), "runtime", config.Config{}, observability.NewMetrics("", ""))
			ctx, cancel := context.WithCancel(t.Context())
			release, err := manager.Acquire(ctx, config.Config{MigrationInstanceID: "test-instance"})
			if err != nil {
				t.Fatal(err)
			}
			...
			if script.execContextErr != nil || manager.Connection() != nil {
				t.Fatalf("release context error=%v retained=%v", script.execContextErr, manager.Connection() != nil)
			}
		})
	}
	...
	manager := NewLockManager(migrationDB, engine, engine.SQLDialect().WithSchema("public"), "", config.Config{DatabaseConnectTimeout: time.Second}, nil)
	release, err := manager.Acquire(t.Context(), config.Config{})
	if err != nil {
		t.Fatal(err)
	}
	release()
...
func TestAcquireMigrationLockReportsConnectionQueryAndTimeoutFailures(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			engine := postgres.NewEngine()
			manager := NewLockManager(db, engine, engine.SQLDialect().WithSchema("public"), "", config.Config{DatabaseLockTimeout: test.timeout}, nil)
			if _, err := manager.Acquire(t.Context(), config.Config{MigrationInstanceID: "test-instance"}); err == nil || !strings.Contains(err.Error(), test.want) {
...
func TestAcquireMigrationLockWaitsThenSucceeds(t *testing.T) {
	...
	db := openMigrationLockScript(t, script)
	engine := postgres.NewEngine()
	manager := NewLockManager(db, engine, engine.SQLDialect().WithSchema("public"), "", config.Config{DatabaseLockTimeout: time.Second}, nil)
	release, err := manager.Acquire(t.Context(), config.Config{MigrationInstanceID: "wait-success"})
	if err != nil {
		t.Fatal(err)
	}
	release()
