...
const (
	...
	ArtifactIdentityManifest   = "identity_manifest"
	ArtifactSigningKeyVersions = "signing_key_versions"
	ArtifactDataKeyVersions    = "data_key_versions"
	ArtifactService            = "service_artifact"
)
...
type BackupEvidence struct {
	...
	DeploymentReferenceHash string     `json:"deployment_reference_hash,omitempty"`
...
func (e BackupEvidence) Validate(now time.Time) error {
	...
	for _, required := range []string{ArtifactDatabase, ArtifactIdentityManifest, ArtifactSigningKeyVersions, ArtifactDataKeyVersions, ArtifactService} {
		if !kinds[required] {
...
// IdentityRestoreReconciliationPlan describes the Identity-owned state that
// operators must reconcile after restoring an older database snapshot. It
...
func (e DrillEvidence) Validate() error {
	...
	for _, required := range []string{"checksum", "schema", "critical_tables", "workspace_counts", "subject_counts", "role_assignments", "catalog_hashes", "session_disposition", "no_secret_export"} {
