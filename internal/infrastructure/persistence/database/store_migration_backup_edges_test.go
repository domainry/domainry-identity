...
func TestApplicationTablesDialectAndFailureEdges(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			state := &databaseSQLState{querySteps: []databaseSQLQueryStep{{columns: []string{"name"}, rows: [][]driver.Value{{"zeta"}, {"_identity_file_migrations"}, {"alpha"}, {"_identity_schema_migrations"}, {""}}}}}
...
func TestSQLiteMigrationBackupChecksumAndStatFailures(t *testing.T) {
	...
	store := &IdentityStore{db: db, dialect: sqlite.Dialect{}, backupChecksum: func(string) (string, error) { return "", errDatabaseSQL }}
