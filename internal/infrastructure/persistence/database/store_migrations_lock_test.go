...
func TestAcquireMigrationLockCoversExternalDialectsAndRelease(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			store := &IdentityStore{db: db, dialect: test.dialect, databaseSchema: "runtime", operationalMetrics: NewIdentityOperationalMetrics("", "")}
	...
	store := &IdentityStore{migrationDB: migrationDB, dialect: postgres.NewEngine(), config: config.Config{DatabaseConnectTimeout: time.Second}}
...
func TestAcquireMigrationLockReportsConnectionQueryAndTimeoutFailures(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			store := &IdentityStore{db: db, dialect: postgres.NewEngine(), config: config.Config{DatabaseLockTimeout: test.timeout}}
...
func TestAcquireMigrationLockWaitsThenSucceeds(t *testing.T) {
	...
	db := openMigrationLockScript(t, script)
	store := &IdentityStore{db: db, dialect: postgres.NewEngine(), config: config.Config{DatabaseLockTimeout: time.Second}}
