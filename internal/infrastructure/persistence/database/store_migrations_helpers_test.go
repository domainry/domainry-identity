...
func TestMigrationHelpersCoverDialectAndFilesystemEdges(t *testing.T) {
	...
	dir := t.TempDir()
	if paths, err := (&IdentityStore{dialect: sqlite.Dialect{}}).migrationPaths(config.Config{MigrationDir: filepath.Join(dir, "missing")}); err != nil || paths != nil {
	...
	if _, err := (&IdentityStore{dialect: sqlite.Dialect{}}).migrationPaths(config.Config{MigrationDir: blocked}); err == nil {
...
func TestMigrationBackupAndTableDiscoveryRejectInvalidInputs(t *testing.T) {
	...
	store := &IdentityStore{dialect: sqlite.Dialect{}}
	...
	unsupported := &IdentityStore{dialect: unsupportedMigrationDialect{}}
...
func TestEnsureMigrationBackupCoversEmptyAndExistingSQLiteDatabases(t *testing.T) {
	...
	emptyStore := &IdentityStore{db: emptyDB, dialect: sqlite.Dialect{}}
	...
	store := &IdentityStore{db: db, dialect: sqlite.Dialect{}, operationalMetrics: NewIdentityOperationalMetrics("", "")}
	...
	store = &IdentityStore{db: db, dialect: sqlite.Dialect{}}
