...
const identityWorkspaceRLSPolicyLedger = "_identity_rls_policies"
...
// every existing Identity table that owns a workspace_id column. RLS remains
...
func (s *IdentityStore) applyWorkspaceRLSPolicies(ctx context.Context) error {
	tables, err := s.workspaceTables(ctx, s.migrationDB)
	if err != nil {
		return err
	}
	applicationRole := s.identifier(s.postgresCapabilities.User)
	policy := s.identifier("domainry_identity_workspace_" + CurrentIdentityWorkspaceRLSPolicyVersion)
	for _, table := range tables {
		...
		statements := []string{
			...
			"CREATE POLICY " + policy + " ON " + relation + " FOR ALL TO " + applicationRole +
	...
	ledger := s.tableIdentifier(identityWorkspaceRLSPolicyLedger)
	...
	if _, err := s.migrationDB.ExecContext(ctx, "INSERT INTO "+ledger+" ("+s.identifier("version")+") VALUES ("+s.placeholder(1)+") ON CONFLICT ("+s.identifier("version")+") DO NOTHING", CurrentIdentityWorkspaceRLSPolicyVersion); err != nil {
		return fmt.Errorf("record workspace RLS policy version: %w", err)
	}
	return nil
}

func (s *IdentityStore) inspectWorkspaceRLS(ctx context.Context) (WorkspaceRLSStatus, error) {
	tables, err := s.workspaceTables(ctx, s.db)
	if err != nil {
		return WorkspaceRLSStatus{}, err
	}
	status := WorkspaceRLSStatus{Enabled: true, Forced: true, ApplicationRole: s.postgresCapabilities.User, PolicyVersion: CurrentIdentityWorkspaceRLSPolicyVersion}
	policyName := "domainry_identity_workspace_" + CurrentIdentityWorkspaceRLSPolicyVersion
	for _, table := range tables {
...
WHERE n.nspname = $1 AND c.relname = $2`, s.DatabaseSchema(), table, policyName).Scan(&enabled, &forced, &roleOwnsTable, &roleBypassRLS, &policyExists)
	...
	if err := s.db.QueryRowContext(ctx, "SELECT COUNT(*) FROM "+s.tableIdentifier(identityWorkspaceRLSPolicyLedger)+" WHERE "+s.identifier("version")+" = "+s.placeholder(1), CurrentIdentityWorkspaceRLSPolicyVersion).Scan(&versionCount); err != nil {
		return WorkspaceRLSStatus{}, fmt.Errorf("verify workspace RLS policy version %s: %w", CurrentIdentityWorkspaceRLSPolicyVersion, err)
	}
	if versionCount != 1 {
		return WorkspaceRLSStatus{}, fmt.Errorf("verify workspace RLS policy version %s: missing ledger entry", CurrentIdentityWorkspaceRLSPolicyVersion)
	}
	return status, nil
}

func (s *IdentityStore) workspaceTables(ctx context.Context, db *sql.DB) ([]string, error) {
