...
func TestDialectContract(t *testing.T) {
	...
	if dsn, err := dialect.DSN(config.Config{DatabaseDSN: " user:password@tcp(localhost:3306)/identity?parseTime=true "}); err != nil || dsn != "user:password@tcp(localhost:3306)/identity?parseTime=true" {
