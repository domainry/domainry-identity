...
func TestNormalizeFieldMutationOwnsAllowedTypeAndRelationCardinality(t *testing.T) {
	...
	_, err := MetadataNormalizeFieldMutation(request, objects, []string{"text", "relation"}, 0)
...
func TestNormalizeFieldMutationPublishesExactCurrencyContract(t *testing.T) {
	...
	request := metadatamodel.MetadataDefinitionUpsertRequest{ObjectKey: "order", Payload: payload}
	normalized, err := MetadataNormalizeFieldMutation(request, objects, []string{"currency"}, 0)
	if err != nil {
		t.Fatal(err)
	}
	...
	request.Payload = payload
	if _, err := MetadataNormalizeFieldMutation(request, objects, []string{"currency"}, 0); err != nil {
		t.Fatal(err)
	}
	for _, testCase := range []struct {
		...
		request.Payload = payload
		_, err = MetadataNormalizeFieldMutation(request, objects, []string{"currency"}, 0)
