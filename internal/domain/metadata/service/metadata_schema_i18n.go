...
func (s *MetadataSchemaDomainService) ForPrincipalLocale(ctx context.Context, principal identitymodel.Principal, locale string) metadatamodel.MetadataSchemaSnapshot {
	...
	for index := range snapshot.Roles {
		...
		role.Name = localize("role", role.Key, "name", role.Name)
	}
