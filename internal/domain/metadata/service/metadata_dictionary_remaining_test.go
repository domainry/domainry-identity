...
func TestDictionaryServiceRemainingValidationCacheAndLocalization(t *testing.T) {
	...
	if _, _, err := service.Items(t.Context(), dictionaryFailureRepository{}, "status", "", identitymodel.Principal{}); err == nil {
		t.Fatal("unknown principal accepted")
	}
	principal := identitymodel.Principal{Known: true, WorkspaceID: "workspace"}
