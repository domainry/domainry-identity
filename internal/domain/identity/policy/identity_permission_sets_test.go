...
func TestExpandRolePermissionSetsComposesGroupsDeterministically(t *testing.T) {
	...
	roles := []identitymodel.RoleSchema{{Key: "operator", Permissions: identitymodel.RolePermissionsWithScope(identitymodel.IdentityDataScopeOwner, "case.resolve"), PermissionSetKeys: []string{"case_read"}, PermissionSetGroups: []string{"case_operator"}}}
	...
	if len(first) != 1 ||
		!reflect.DeepEqual(first[0].Permissions, identitymodel.RolePermissionsWithScope(identitymodel.IdentityDataScopeOwner, "case.resolve")) ||
...
func TestExpandRolePermissionSetsIgnoresUnknownReferencesWithoutAddingAuthority(t *testing.T) {
	...
	if len(roles) != 1 || len(roles[0].Permissions) != 0 {
...
func TestExpandRoleAuthorizationAttachesReusableGuardrail(t *testing.T) {
	...
	roles := ExpandRoleAuthorization(
		[]identitymodel.RoleSchema{{Key: "operator", Permissions: identitymodel.RolePermissionsWithScope(identitymodel.IdentityDataScopeAll, "case.export"), GuardrailKeys: []string{"no_export"}}},
		nil,
		nil,
