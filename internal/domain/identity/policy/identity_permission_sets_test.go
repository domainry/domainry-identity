...
func TestExpandRolePermissionSetsComposesGroupsDeterministically(t *testing.T) {
	sets := []identitymodel.IdentityPermissionSet{
		{Key: "case_read", Permissions: []string{"case.read"}, DataPermissions: []identitymodel.DataPermission{{ObjectKey: "case", Scope: "owned_records", Read: true}}},
		{
			Key: "case_department", Permissions: []string{"case.read", "case.resolve"},
	...
	roles := []identitymodel.RoleSchema{{Key: "operator", PermissionSetKeys: []string{"case_read"}, PermissionSetGroups: []string{"case_operator"}}}
	...
	if len(first) != 1 ||
		!reflect.DeepEqual(first[0].Permissions, []string{"case.read", "case.resolve"}) ||
