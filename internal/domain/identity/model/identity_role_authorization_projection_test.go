...
func TestIdentityExpandRoleAuthorizationBlankAndMissingKeys(t *testing.T) {
	roles := []RoleSchema{{
		Permissions:         RolePermissionsWithScope(IdentityDataScopeAll, "role.read"),
		PermissionSetKeys:   []string{" ", "direct"},
		PermissionSetGroups: []string{"group", "missing"},
		GuardrailKeys:       []string{"guard", "missing"},
	}}
	sets := []IdentityPermissionSet{{Key: " "}, {Key: "direct"}, {Key: "grouped", FieldPermissions: []FieldPermission{{ObjectKey: "grouped", FieldKey: "name", Read: true}}}}
	...
	if len(got) != 1 || len(got[0].Permissions) != 1 || got[0].Permissions[0].PermissionKey != "role.read" || len(got[0].FieldPermissions) != 1 || len(got[0].Guardrails) != 2 {
