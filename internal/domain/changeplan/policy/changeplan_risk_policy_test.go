...
func TestChangePlanSensitiveUpdateAndRiskHelpers(t *testing.T) {
	...
	for _, resourceType := range []string{"role", "permission", "data_scope", "field_permission", "menu", "identity.role", "identity.role_permission", "identity.role_data_scope", "identity.role_field_permission", "identity.menu"} {
...
func TestChangePlanCanonicalResourceType(t *testing.T) {
	for input, want := range map[string]string{" identity.role ": "identity.role", " field ": "field", "": ""} {
