...
func TestChangePlanReferenceImpactDirectIndirectCycleAndStableOrder(t *testing.T) {
	...
	graph := changeplanmodel.ReferenceGraph{Hash: "hash-1", Edges: []changeplanmodel.ReferenceEdge{
		...
		{FromType: "view", FromKey: "v", ToType: "action", ToKey: "a", Kind: "invokes"},
	...
	if len(impact.IndirectConsumers) != 3 || impact.IndirectConsumers[0].FromKey != "other" || impact.IndirectConsumers[1].FromKey != "s" || impact.IndirectConsumers[2].FromKey != "v" {
