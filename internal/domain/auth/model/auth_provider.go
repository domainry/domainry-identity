...
func AuthProviderConfigFromMap(raw map[string]any) AuthProviderConfig {
	...
	config := AuthProviderConfig{Key: get("key"), Label: get("label"), Type: get("type"), Enabled: getBool("enabled"), Issuer: get("issuer"), AuthURL: get("auth_url"), TokenURL: get("token_url"), UserInfoURL: get("userinfo_url"), ClientID: get("client_id"), ClientSecret: get("client_secret"), RedirectURL: get("redirect_url"), Scope: get("scope"), OTPProvider: get("otp_provider"), AccessToken: get("access_token"), PhoneNumberID: get("phone_number_id"), AutoCreateUsers: getBool("auto_create_users"), AutoCreateConfigured: autoCreateConfigured, DefaultRoleKey: get("default_role_key"), ClientSecretConfigured: getBool("client_secret_configured"), AccessTokenConfigured: getBool("access_token_configured"), PhoneNumberIDConfigured: getBool("phone_number_id_configured"), ConfiguredFrom: get("configured_from"), UpdatedAt: get("updated_at"), Extra: map[string]any{}}
...
func (c AuthProviderConfig) Map() map[string]any {
	...
	for key, value := range map[string]any{"key": c.Key, "label": c.Label, "type": c.Type, "enabled": c.Enabled, "issuer": c.Issuer, "auth_url": c.AuthURL, "token_url": c.TokenURL, "userinfo_url": c.UserInfoURL, "client_id": c.ClientID, "client_secret": c.ClientSecret, "redirect_url": c.RedirectURL, "scope": c.Scope, "otp_provider": c.OTPProvider, "access_token": c.AccessToken, "phone_number_id": c.PhoneNumberID, "auto_create_users": c.AutoCreateUsers, "default_role_key": c.DefaultRoleKey, "role_mappings": c.RoleMappings, "client_secret_configured": c.ClientSecretConfigured, "access_token_configured": c.AccessTokenConfigured, "phone_number_id_configured": c.PhoneNumberIDConfigured, "configured_from": c.ConfiguredFrom, "updated_at": c.UpdatedAt} {
