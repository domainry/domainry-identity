...
func (s *AuditDomainService) Events(ctx context.Context, query auditmodel.AuditEventQuery, principal identitymodel.Principal) ([]auditmodel.AuditEvent, error) {
	if !principal.Known {
		return nil, auditDomainError(apperror.KindForbidden, "backend.role.unknown", nil)
	}
	if !identitycontract.IdentityRoleHasPermissionKey(principal.Role, "identity.audit.view") && !identitycontract.IdentityRoleAllows(principal.Role, "identity_permission", "read") {
...
func (s *AuditDomainService) Options(ctx context.Context, query auditmodel.AuditOptionQuery, principal identitymodel.Principal) ([]auditmodel.AuditOption, error) {
	if !principal.Known {
		return nil, auditDomainError(apperror.KindForbidden, "backend.role.unknown", nil)
	}
	if !identitycontract.IdentityRoleHasPermissionKey(principal.Role, "identity.audit.view") && !identitycontract.IdentityRoleAllows(principal.Role, "identity_permission", "read") {
...
func (s *AuditDomainService) ListAuditEventsForSystem(ctx context.Context, scope identitymodel.SystemScope, query auditmodel.AuditEventQuery) ([]auditmodel.AuditEvent, error) {
	if err := ctx.Err(); err != nil {
		return nil, err
	}
	if s == nil || s.repository == nil {
...
func (s *AuditDomainService) AppendWithMetadata(ctx context.Context, event, objectKey, recordID string, principal identitymodel.Principal, summary string, before, after, metadata map[string]any) {
