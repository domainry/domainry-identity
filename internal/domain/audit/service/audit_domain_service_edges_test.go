...
func (r *auditEdgeRepository) ListAuditEventsForSystem(context.Context, identitymodel.SystemScope, auditmodel.AuditEventQuery) ([]auditmodel.AuditEvent, error) {
...
func TestAuditQueriesCoverAuthorizationFieldsAndRepositoryFailures(t *testing.T) {
	...
	unknown := identitymodel.Principal{}
	denied := identitymodel.Principal{Known: true}
	legacyReader := identitymodel.Principal{Known: true, WorkspaceID: " workspace-a ", Role: identitymodel.RoleSchema{Permissions: []string{"identity_permission.read"}}}
...
func TestAuditReadersCoverCancellationUnavailableAndRepositoryResults(t *testing.T) {
	...
	if _, err := service.ListAuditEventsForSystem(t.Context(), identitymodel.SystemScope{}, auditmodel.AuditEventQuery{}); !errors.Is(err, want) {
