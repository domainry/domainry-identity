...
func TestMandatoryAuditReturnsStorageFailureWhileTelemetryIsBestEffort(t *testing.T) {
	...
	service := NewAuditDomainService(repository)
	request := auditcontract.AuditAppendRequest{Event: "record_created", Principal: identitymodel.Principal{Known: true}}
...
func TestAuditEventFactoryRedactsAndAddsPrincipalContext(t *testing.T) {
	...
	event := service.NewAuditEvent(t.Context(), auditcontract.AuditAppendRequest{
		Event: "integration_called", Principal: identitymodel.Principal{UserID: "user-1", WorkspaceID: "workspace-1", RequestID: "request-1", Role: identitymodel.RoleSchema{Key: "operator"}},
	...
	next := service.NewAuditEvent(t.Context(), auditcontract.AuditAppendRequest{Event: "integration_called", Principal: identitymodel.Principal{WorkspaceID: "workspace-1"}})
	if next.ID == event.ID {
...
func TestIdempotentAuditEventIgnoresPerAttemptRequestID(t *testing.T) {
	...
	request := auditcontract.AuditAppendRequest{
		...
		Principal:      identitymodel.Principal{UserID: "user-1", WorkspaceID: "workspace-1", RequestID: "attempt-1", Role: identitymodel.RoleSchema{Key: "operator"}},
...
func (r *repositoryStub) ListAuditEventsForSystem(context.Context, identitymodel.SystemScope, auditmodel.AuditEventQuery) ([]auditmodel.AuditEvent, error) {
...
func TestServiceUsesOnlyAuditRepository(t *testing.T) {
	...
	admin := identitymodel.Principal{Known: true, UserID: "admin", Role: identitymodel.RoleSchema{Permissions: []string{"identity.audit.view"}}}
	...
	if _, err := application.Events(t.Context(), auditmodel.AuditEventQuery{}, identitymodel.Principal{}); errorCode(err) != "backend.role.unknown" {
...
func TestAuditQueriesEnforcePermissionsAndWrapRepositoryFailures(t *testing.T) {
	...
	service := NewAuditDomainService(repository)
	withoutPermission := identitymodel.Principal{Known: true, WorkspaceID: "workspace-1"}
	...
	if _, err := service.Options(t.Context(), auditmodel.AuditOptionQuery{Field: "event"}, identitymodel.Principal{}); errorCode(err) != "backend.role.unknown" {
	...
	legacyReader := identitymodel.Principal{Known: true, WorkspaceID: "workspace-1", Role: identitymodel.RoleSchema{Permissions: []string{"identity_permission.read"}}}
...
func TestAuditReaderPortsHandleCancellationAvailabilityAndSystemScope(t *testing.T) {
	...
	if _, err := service.ListAuditEventsForSystem(cancelled, identitymodel.SystemScope{}, auditmodel.AuditEventQuery{}); !errors.Is(err, context.Canceled) {
	...
	if _, err := nilService.ListAuditEventsForSystem(t.Context(), identitymodel.SystemScope{}, auditmodel.AuditEventQuery{}); errorCode(err) != "backend.audit.repository_unavailable" {
	...
	if _, err := unavailable.ListAuditEventsForSystem(t.Context(), identitymodel.SystemScope{}, auditmodel.AuditEventQuery{}); errorCode(err) != "backend.audit.repository_unavailable" {
	...
	if events, err := service.ListAuditEventsForSystem(t.Context(), identitymodel.SystemScope{}, auditmodel.AuditEventQuery{}); err != nil || len(events) != 1 {
