...
import (
	...
	auditpolicy "github.com/domainry/domainry-identity/internal/domain/audit/policy"
)
...
func newAuditEvent(ctx context.Context, request auditcontract.AuditAppendRequest) auditmodel.AuditEvent {
	...
	if principal.Role.Key != "" {
		metadata["role_key"] = principal.Role.Key
	}
	now := time.Now().UTC()
	...
	return auditmodel.AuditEvent{
		...
		ActorID: valueOrDefault(principal.UserID, "system"), RoleKey: principal.Role.Key,
...
func principalWorkspaceID(principal identitymodel.Principal) string {
