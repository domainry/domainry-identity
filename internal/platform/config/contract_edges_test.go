...
func TestConfigContractTypedSettersAndCollectionParsers(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{Definition{Field: "AuthAllowDevHeaders", Type: TypeBool}, " true ", func(t *testing.T, cfg Config) {
			t.Helper()
			if !cfg.AuthAllowDevHeaders {
				t.Fatal("bool not set")
			}
		}},
		{Definition{Field: "HTTPMaxHeaderBytes", Type: TypeInt}, " 42000 ", func(t *testing.T, cfg Config) {
			t.Helper()
			if cfg.HTTPMaxHeaderBytes != 42000 {
				t.Fatalf("int=%d", cfg.HTTPMaxHeaderBytes)
			}
		}},
		...
		{Definition{Field: "AuthAccessTTL", Type: TypeDuration}, " 90s ", func(t *testing.T, cfg Config) {
			t.Helper()
			if cfg.AuthAccessTTL != 90*time.Second {
				t.Fatalf("duration=%v", cfg.AuthAccessTTL)
			}
		}},
	...
	for _, test := range []struct {
		...
		{Definition{Field: "AuthAllowDevHeaders", Type: TypeBool}, "not-bool"},
		{Definition{Field: "HTTPMaxHeaderBytes", Type: TypeInt}, "not-int"},
		{Definition{Field: "TelemetrySampleRatio", Type: TypeFloat}, "not-float"},
		{Definition{Field: "AuthAccessTTL", Type: TypeDuration}, "not-duration"},
...
func TestConfigContractProvenanceReportAndImplicitVersions(t *testing.T) {
	cfg, snapshot, err := LoadContract(Source{Name: "runtime-source", Priority: 10, Values: map[string]string{
		"PORT": "9091", "AUTH_JWT_SECRET": "runtime-secret", "TELEMETRY_HEADERS": "tenant=workspace-a",
	}})
	if err != nil {
		t.Fatal(err)
	}
	if cfg.Port != "9091" || cfg.AuthJWTSecret != "runtime-secret" || cfg.TelemetryHeaders["tenant"] != "workspace-a" {
		t.Fatalf("cfg=%+v", cfg)
	}
	if snapshot.Entries["AUTH_JWT_SECRET"].Version != "unversioned" || !snapshot.Entries["AUTH_JWT_SECRET"].Redacted || snapshot.Entries["PORT"].Version != valueVersion("9091") {
...
func TestConfigValidationRejectsEachRuntimeBoundary(t *testing.T) {
	...
	tests := []struct {
		...
		{"auth TTL", func(cfg *Config) { cfg.AuthRefreshTTL = cfg.AuthAccessTTL }, "AUTH_REFRESH_TTL"},
		...
		{"required paths", func(cfg *Config) { cfg.ManifestPath = "" }, "paths are required"},
	}
...
func TestConfigLoadSourceAndSecretFileErrors(t *testing.T) {
	...
	t.Run("missing secret file", func(t *testing.T) {
		...
		t.Setenv("AUTH_JWT_SECRET_FILE", filepath.Join(t.TempDir(), "missing.secret"))
		if _, _, err := Load(); err == nil || !strings.Contains(err.Error(), "AUTH_JWT_SECRET_FILE") {
			t.Fatalf("error=%v", err)
		}
	})
	t.Run("secret file", func(t *testing.T) {
		...
		if err != nil || cfg.AuthJWTSecret != "file-secret" || snapshot.Entries["AUTH_JWT_SECRET"].Source != "secret-file" {
			t.Fatalf("secret=%q entry=%+v err=%v", cfg.AuthJWTSecret, snapshot.Entries["AUTH_JWT_SECRET"], err)
		}
	})
	t.Run("invalid environment value", func(t *testing.T) {
		t.Setenv("RUNTIME_CONFIG_FILE", "")
		t.Setenv("RUNTIME_REMOTE_CONFIG_FILE", "")
		t.Setenv("HTTP_MAX_HEADER_BYTES", "not-int")
		if _, _, err := Load(); err == nil || !strings.Contains(err.Error(), "HTTP_MAX_HEADER_BYTES") {
			t.Fatalf("error=%v", err)
		}
	})
}
...
func TestConfigManagedNamesAndSecurityValidationEdges(t *testing.T) {
	...
	if !managedConfigName("PORT") || !managedConfigName("AUTH_JWT_SECRET") || managedConfigName("PATH") {
	...
	if err := valid.Validate(); err == nil || !strings.Contains(err.Error(), "AUTH_ALLOW_DEV_HEADERS") {
