...
func setValidProductionListeners(cfg *Config) {
	...
	cfg.HTTPManagementMaxJSONBodyBytes = 2 << 20
	cfg.HTTPOpsMaxJSONBodyBytes = 1 << 20
	cfg.HTTPPublicRequestTimeout = 30 * time.Second
	cfg.HTTPManagementRequestTimeout = 30 * time.Second
	cfg.HTTPOpsRequestTimeout = 15 * time.Second
	cfg.HTTPPublicRateLimitPerMinute = 6000
	cfg.HTTPManagementRateLimitPerMinute = 3000
	cfg.HTTPOpsRateLimitPerMinute = 1200
}

func TestProductionHTTPControlsRejectDisabledLimits(t *testing.T) {
	valid := Config{
		HTTPPublicMaxJSONBodyBytes: 1, HTTPManagementMaxJSONBodyBytes: 1, HTTPOpsMaxJSONBodyBytes: 1,
		HTTPPublicRequestTimeout: time.Second, HTTPManagementRequestTimeout: time.Second, HTTPOpsRequestTimeout: time.Second,
		HTTPPublicRateLimitPerMinute: 1, HTTPManagementRateLimitPerMinute: 1, HTTPOpsRateLimitPerMinute: 1,
	}
...
func TestValidateProductionSurfaceListeners(t *testing.T) {
	...
	if err := valid.validateProductionListenerGroups(); err != nil {
		t.Fatalf("valid listener groups: %v", err)
	}
	tests := []struct {
		name   string
		mutate func(*Config)
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			cfg := valid
			test.mutate(&cfg)
			if err := cfg.validateProductionListenerGroups(); err == nil {
	...
	if err := breakGlass.validateProductionListenerGroups(); err != nil {
...
func TestProductionSurfaceValidationRemainingConditions(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			err := cfg.validateProductionListenerGroups()
	...
	if err := missingReason.validateProductionListenerGroups(); err == nil {
