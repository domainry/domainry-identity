...
func metadataInstallationScope(purpose string) identitymodel.SystemScope {
	return identitymodel.NewSystemScope(identitymodel.SystemScopeInstallation, purpose)
}

func metadataAuthorizeQuery(principal identitymodel.Principal) error {
...
func metadataAuthorizeCommand(principal identitymodel.Principal) error {
...
func metadataAuthorizeWorkspaceQuery(workspaceID string) error {
	if _, err := identitymodel.NewWorkspaceQueryScope(workspaceID); err != nil {
		return &apperror.AppError{Kind: apperror.KindForbidden, Code: "backend.workspace_scope_required", Err: err}
	}
	return nil
}
