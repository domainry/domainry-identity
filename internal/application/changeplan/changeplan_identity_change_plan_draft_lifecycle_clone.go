...
func (s *ChangePlanApplicationService) Draft(ctx context.Context, planID string, principal identitymodel.Principal) (changeplanmodel.BusinessChangePlanDraft, error) {
...
func (s *ChangePlanApplicationService) SaveDraft(ctx context.Context, plan changeplanmodel.BusinessSystemChangePlan, expectedRevision int, principal identitymodel.Principal) (changeplanmodel.BusinessChangePlanDraft, error) {
	...
	audit := buildAuditEvent("identity_change_plan.draft_saved", "identity_change_plan", plan.PlanID, principal, "Saved domain change plan draft", nil, map[string]any{"revision": saved.Revision}, map[string]any{"business_reason": plan.BusinessReason, "builder_task_id": plan.BuilderTaskID, "change_plan_id": plan.PlanID})
...
func (s *ChangePlanApplicationService) SubmitDraftForReview(ctx context.Context, planID string, expectedRevision int, snapshot changeplancontract.SnapshotSource, graph changeplancontract.ReferenceGraphSource, principal identitymodel.Principal) (changeplanmodel.BusinessChangePlanDraft, changeplanmodel.BusinessChangePlanValidation, error) {
	if err := changePlanAuthorizeCommand(principal); err != nil {
...
func (s *ChangePlanApplicationService) PublishApprovedDraftIdempotent(ctx context.Context, planID string, expectedRevision int, confirmation, operationKey string, snapshot changeplancontract.SnapshotSource, graph changeplancontract.ReferenceGraphSource, principal identitymodel.Principal) (BusinessChangePlanApplyResult, bool, error) {
	if err := changePlanAuthorizeCommand(principal); err != nil {
...
func (s *ChangePlanApplicationService) approvedDraftForPublish(ctx context.Context, planID string, expectedRevision int, principal identitymodel.Principal) (changeplanmodel.BusinessChangePlanDraft, changeplanmodel.BusinessSystemChangePlan, error) {
...
func (s *ChangePlanApplicationService) reviewableDraft(ctx context.Context, planID string, expectedRevision int, status string, principal identitymodel.Principal) (changeplanmodel.BusinessChangePlanDraft, changeplanmodel.BusinessSystemChangePlan, error) {
	planID = strings.TrimSpace(planID)
	if planID == "" {
		return changeplanmodel.BusinessChangePlanDraft{}, changeplanmodel.BusinessSystemChangePlan{}, badRequest("backend.change_plan.plan_id_required")
	}
...
func (s *ChangePlanApplicationService) validateReviewedDraft(ctx context.Context, plan changeplanmodel.BusinessSystemChangePlan, reviewer string, snapshot changeplancontract.SnapshotSource, graph changeplancontract.ReferenceGraphSource, principal identitymodel.Principal) (changeplanmodel.BusinessChangePlanValidation, error) {
...
func (s *ChangePlanApplicationService) transitionDraft(ctx context.Context, draft changeplanmodel.BusinessChangePlanDraft, fromStatus, toStatus string, principal identitymodel.Principal) (changeplanmodel.BusinessChangePlanDraft, error) {
...
func (s *ChangePlanApplicationService) auditDraftLifecycle(ctx context.Context, draft changeplanmodel.BusinessChangePlanDraft, principal identitymodel.Principal, event string) {
	if s.audit == nil {
		return
	}
	audit := buildAuditEvent("identity_change_plan."+event, "identity_change_plan", draft.PlanID, principal, "Business change plan "+event, nil, map[string]any{"status": draft.Status, "revision": draft.Revision}, map[string]any{"change_plan_id": draft.PlanID, "revision": draft.Revision, "status": draft.Status})
