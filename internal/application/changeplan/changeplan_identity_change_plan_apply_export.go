...
import (
	...
	changeplanvalidation "github.com/domainry/domainry-identity/internal/domain/changeplan/validation"
...
func (s *ChangePlanApplicationService) ApplyIdempotent(ctx context.Context, plan changeplanmodel.BusinessSystemChangePlan, confirmation, operationKey string, snapshot changeplancontract.SnapshotSource, graph changeplancontract.ReferenceGraphSource, principal identitymodel.Principal) (BusinessChangePlanApplyResult, bool, error) {
...
func (s *ChangePlanApplicationService) auditChangePlanIdempotency(ctx context.Context, plan changeplanmodel.BusinessSystemChangePlan, principal identitymodel.Principal, claim changeplanmodel.ChangePlanOperationClaimResult, status string) {
	...
	event := buildAuditEvent("identity_change_plan.idempotency_"+status, "identity_change_plan", plan.PlanID, principal, "Observed idempotent change plan apply", nil, nil, metadata)
...
func changePlanApplyFingerprint(plan changeplanmodel.BusinessSystemChangePlan, confirmation string) (string, error) {
	...
	return idempotency.Fingerprint(idempotency.FingerprintInput{UseCase: "change_plan.apply", ResourceType: "identity_change_plan", TargetID: strings.TrimSpace(plan.PlanID), Payload: canonical, Preconditions: map[string]any{"plan_version": plan.PlanVersion, "draft_revision": plan.DraftRevision, "confirmation": strings.TrimSpace(confirmation)}})
}
...
func (s *ChangePlanApplicationService) Apply(ctx context.Context, plan changeplanmodel.BusinessSystemChangePlan, confirmation string, snapshot changeplancontract.SnapshotSource, graph changeplancontract.ReferenceGraphSource, principal identitymodel.Principal) (BusinessChangePlanApplyResult, error) {
	if err := changePlanAuthorizeCommand(principal); err != nil {
	...
	definitions, err := s.metadata.ApplyDefinitionMutations(ctx, identitymodel.NewSystemScope(identitymodel.SystemScopeInstallation, "apply approved metadata definition mutations"), mutations, audits, publication)
	if err != nil {
...
func (s *ChangePlanApplicationService) Validate(_ context.Context, plan changeplanmodel.BusinessSystemChangePlan, snapshot changeplancontract.SnapshotSource, graph changeplancontract.ReferenceGraphSource, principal identitymodel.Principal) (changeplanmodel.BusinessChangePlanValidation, error) {
...
func (s *ChangePlanApplicationService) auditBusinessChangePlanResult(ctx context.Context, plan changeplanmodel.BusinessSystemChangePlan, principal identitymodel.Principal, result, errorCode string, validation *changeplanmodel.BusinessChangePlanValidation) {
	...
	event := buildAuditEvent("identity_change_plan."+result, "identity_change_plan", plan.PlanID, principal, "Business change plan "+result, nil, nil, metadata)
	_ = s.audit.InsertAuditEvent(ctx, event.WorkspaceID, event)
}

func (s *ChangePlanApplicationService) ExportPackage(ctx context.Context, planID string, expectedRevision int, principal identitymodel.Principal) (changeplanmodel.BusinessSystemPackage, error) {
