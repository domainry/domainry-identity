...
func (repository *workspaceGuardAuditRepository) ListAuditEventsForSystem(context.Context, identitymodel.SystemScope, auditmodel.AuditEventQuery) ([]auditmodel.AuditEvent, error) {
	repository.calls++
	return nil, nil
}
...
func TestAuditApplicationAuthorizesWorkspaceBeforeRepositoryAccess(t *testing.T) {
	...
	principal := identitymodel.Principal{Known: true, Role: identitymodel.RoleSchema{Permissions: []string{"identity.audit.view"}}}
