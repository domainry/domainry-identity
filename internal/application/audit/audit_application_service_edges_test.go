...
func (r *auditApplicationEdgeRepository) ListAuditEventsForSystem(context.Context, identitymodel.SystemScope, auditmodel.AuditEventQuery) ([]auditmodel.AuditEvent, error) {
	return r.events, r.err
}
...
func TestAuditApplicationHappyErrorAndHelpers(t *testing.T) {
	...
	principal := identitymodel.Principal{Known: true, WorkspaceID: " workspace-1 ", UserID: "user-1", Role: identitymodel.RoleSchema{Permissions: []string{"identity.audit.view"}}}
	...
	service.SetEventProjector(func(_ context.Context, events []auditmodel.AuditEvent, _ identitymodel.Principal) ([]auditmodel.AuditEvent, error) {
	...
	if AuditPrincipalWorkspaceID(principal) != "workspace-1" || AuditPrincipalWorkspaceID(identitymodel.Principal{}) != "default" {
