...
Module 接入必须把每个可执行入口声明成 source-owned `ActionDefinition`。宿主从 Action 提取角色可配置的同 key `PermissionDefinition`，经 Identity reconcile 成功后才允许 adapter ready。
...
| `SourceKind` | module HTTP adapter 使用 `module_http` |
| `CapabilityKey/Label` | 配置页的功能分组及用户可读名称 |
| `OperationKey/Label` | list/get/create/update/approve 等精确语义 |
| `Exposures` | `public`、`management`、`ops` 中显式选择；这是监听器暴露范围，不是前端产品壳 |
...
action.ActionDefinition{
    ...
    SourceKind:    "module_http",
    CapabilityKey: "notification.management",
    OperationKey:  "list",
    Exposures:      []action.Exposure{action.ExposureManagement},
    Authorization:  action.Authorization{Strategy: action.AuthorizationAuthenticated},
    HTTP:           &action.HTTPBinding{Method: "GET", RouteTemplate: "/notification/templates"},
...
1. `modulehttp.Adapter.Routes()`：对具有 HTTP binding 的 Action 使用 `modulehttp.RouteFromAction`。
...
build and validate candidate ActionRegistry
  ...
  -> mount adapters and report ready
```
...
- `ProductRoutes()`、Adapter、OpenAPI 和 SaaS Permission reconcile 都从该 manifest 投影；
...
| --- | --- |
| `GET /notification/templates` | `notification.templates.list` |
| `GET /notification/templates/{templateKey}` | `notification.templates.get` |
| `PUT /notification/templates/{templateKey}/draft` | `notification.templates.draft.save` |
| `POST /notification/templates/{templateKey}/preview` | `notification.templates.preview` |
| `POST /notification/templates/{templateKey}/publication-requests` | `notification.publications.request` |
| `GET /notification/publications` | `notification.publications.list` |
| `POST /notification/publications/{publicationID}/approve` | `notification.publications.approve` |
| `POST /notification/publications/{publicationID}/reject` | `notification.publications.reject` |
| `POST /notification/publications/{publicationID}/cancel` | `notification.publications.cancel` |
...
- [ ] Action manifest、实际 handler、Adapter routes 和 OpenAPI 集合完全一致。
